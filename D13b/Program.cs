...
namespace D13b
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Int64 score = 0;
            steps[0] = 2;
            (Int64 x, Int64 y) paddlePos = (0, 0);
            (Int64 x, Int64 y) ballPos = (0, 0);

            while (true)
            {
                Console.Write("\rScore = " + score.ToString());
                
                IntProgram(paddlePos.x > ballPos.x ? -1 : paddlePos.x < ballPos.x ? 1 : 0, ref x, ref index, ref status);
                if (status == 1)
                    break;
                IntProgram(paddlePos.x > ballPos.x ? -1 : paddlePos.x < ballPos.x ? 1 : 0, ref y, ref index, ref status);
                if (status == 1)
                    break;
                IntProgram(paddlePos.x > ballPos.x ? -1 : paddlePos.x > ballPos.x ? 1 : 0, ref type, ref index, ref status);
                if (status == 1)
                {
                    if (x == -1 && y == 0)
                    {
